...
import ga.ziemniaki.cebulionwarenext.client.modules.Ping;
...
public abstract class EntityLivingBase extends Entity
{
    ...
    public void func_191986_a(float p_191986_1_, float p_191986_2_, float p_191986_3_)
    {
        if (this.isServerWorld() || this.canPassengerSteer())
        {
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
            {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying)
                {
                    if (this.isElytraFlying())
                    {
                        ...
                        float rotationPitch = this.rotationPitch * 0.017453292F;
                        double lookVecSqrd = Math.sqrt(vec3d.x * vec3d.x + vec3d.z * vec3d.z);
                        ...
                        boolean elytraHack = Jigsaw.getModuleByName("ElytraHack").isToggled(); //TODO Jigsaw elytra hack
                        boolean NCP = Jigsaw.getModuleByName("ElytraHack").isMode("NCP");
                        
                        MovementInput input = Minecraft.getMinecraft().player.movementInput;
                        
                        Vec3d lookVecPlus90 = this.getVectorForRotation(this.rotationPitch, this.rotationYaw + 90);
                        double lookVecSqrdPlus90 = Math.sqrt(lookVecPlus90.x * lookVecPlus90.x + lookVecPlus90.z * lookVecPlus90.z);
                        
                        Vec3d lookVecMinus90 = this.getVectorForRotation(this.rotationPitch, this.rotationYaw - 90);
                        double lookVecSqrdMinus90 = Math.sqrt(lookVecMinus90.x * lookVecMinus90.x + lookVecMinus90.z * lookVecMinus90.z);
                        ...
                        if(elytraHack) {
                        	if(NCP) {
                        		...
                        		if(input.forwardKeyDown) {
                            		double d2 = 0.05;
                                    this.motionX += vec3d.x * d2 / lookVecSqrd;
                                    this.motionZ += vec3d.z * d2 / lookVecSqrd;
                            	}
                            	...
                            	if(input.moveStrafe > 0) {
                            		double d2 = 0.1;
                                    this.motionX += lookVecMinus90.x * d2 / lookVecSqrdMinus90;
                                    this.motionZ += lookVecMinus90.z * d2 / lookVecSqrdMinus90;
                            	}
                            	if(input.moveStrafe < 0) {
                            		double d2 = 0.1;
                                    this.motionX += lookVecPlus90.x * d2 / lookVecSqrdPlus90;
                                    this.motionZ += lookVecPlus90.z * d2 / lookVecSqrdPlus90;
                            	}
                            	if(input.jump) {
                                    this.motionY += 0.1;
                            	}
                        ...
                        if(!elytraHack || (!NCP && elytraHack)) {
                        	if (this.motionY < 0.0D && lookVecSqrd > 0.0D)
                            {
                            ...
                            {
                                double d10 = horizontalVelocity * (double)(-MathHelper.sin(rotationPitch)) * 0.04D;
                                this.motionY += d10 * 3.2D;
                                this.motionX -= vec3d.x * d10 / lookVecSqrd;
                                this.motionZ -= vec3d.z * d10 / lookVecSqrd;
                            }

                            if (lookVecSqrd > 0.0D)
                            {
                                this.motionX += (vec3d.x / lookVecSqrd * horizontalVelocity - this.motionX) * 0.1D;
                                this.motionZ += (vec3d.z / lookVecSqrd * horizontalVelocity - this.motionZ) * 0.1D;
                            }
                        }
                        ...
                        if (this.isCollidedHorizontally && !this.world.isRemote)
                        {
                            ...
                            double d3 = horizontalVelocity - d11;
            ...
            else
            {
                ...
                this.moveFlying(p_191986_1_, p_191986_2_, p_191986_3_, f2);
                this.moveEntity(MoverType.SELF, this.motionX, this.motionY, this.motionZ);
    ...
    {
		// TODO Jigsaw prediction

		lastPredictedPosX = predictedPosX;
		lastPredictedPosY = predictedPosY;
		lastPredictedPosZ = predictedPosZ;

		long timer = lastRecievedPositionTimer.getTicks();
		timer = Ping.lastTime / 200;

		if (this instanceof EntityPlayerSP) {
			// System.out.println(this.posX - this.lastTickPosXNotPos);
		}

		predictedPosX = this.posX + (this.posX - this.lastTickPosXNotPos) * timer;
		predictedPosY = this.posY + (this.posY - this.lastTickPosYNotPos) * timer;
		predictedPosZ = this.posZ + (this.posZ - this.lastTickPosZNotPos) * timer;
