...
import ga.ziemniaki.cebulionwarenext.client.TickTimer;
import ga.ziemniaki.cebulionwarenext.client.main.Jigsaw;
import ga.ziemniaki.cebulionwarenext.client.modules.ScaffoldFly;
...
{
	protected TickTimer lastRecievedPositionTimer = new TickTimer(20);
	
	public double predictionError;
	
	public double predictedPosX;
	public double predictedPosY;
	public double predictedPosZ;
	
	public double lastPredictedPosX;
	public double lastPredictedPosY;
	public double lastPredictedPosZ;
	
	/**
	 * lastTickPos that will never equal the player position
	 */
	public double lastTickPosXNotPos;
	/**
	 * lastTickPos that will never equal the player position
	 */
	public double lastTickPosYNotPos;
	/**
	 * lastTickPos that will never equal the player position
	 */
	public double lastTickPosZNotPos;
	...
	public boolean isMovingXZ(double abs) {
	...
	public boolean isMovingXYZ(double abs) {
    ...
    public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_)
    {
        ...
        else
        {
            ...
            if ((x == MoverType.SELF || x == MoverType.PLAYER) && this.onGround && this.isSneaking() && this instanceof EntityPlayer || ((Jigsaw.getModuleByName("SafeWalk").isToggled() || ScaffoldFly.doSafeWalk()) && this instanceof EntityPlayer && this.onGround))
            {
    ...
    public void moveFlying(float strafe, float jump, float forward, float friction)
    {
        float f = strafe * strafe + jump * jump + forward * forward;
