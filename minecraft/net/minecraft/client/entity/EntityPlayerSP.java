...
public class EntityPlayerSP extends AbstractClientPlayer
{
	...
	public void moveEntity(MoverType x, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
		// TODO atlas premotion
		PreMotionEvent event = new PreMotionEvent(p_70091_2_, p_70091_4_, p_70091_6_);
		Jigsaw.onPreMotion(event);
		if (event.isCancelled()) {
			return;
		}
		double d0 = this.posX;
		double d1 = this.posZ;
		super.moveEntity(x, event.x, event.y, event.z);
    ...
    {
		// TODO Jigsaw onUpdate
		UpdateEvent event = new UpdateEvent(this.rotationYaw, this.rotationPitch, this.posX, this.posY, this.posZ,
				this.onGround);
		Jigsaw.OnUpdate(event);
		if (event.isCancelled()) {
			return;
		}
        ...
        if (this.isCurrentViewEntity()) //TODO Jigsaw event manipulation thing
        {
            ...
            {
                this.connection.sendPacket(new CPacketPlayer.PositionRotation(event.x, event.y, event.z,
						event.yaw, event.pitch, event.onGround));
            }
            else if (flag2)
            {
    ...
    {
    	if(Jigsaw.getModuleByName("NoSwing").isToggled()) { //TODO: Jigsaw noswing
    		return;
    	}
        super.swingArm(hand);
    ...
    {
		// TODO Jigsaw onLivingUpdate
		Jigsaw.onLivingUpdate();
        ...
        if (!Jigsaw.getModuleByName("NoSlowdown").isToggled() && this.isHandActive() && !this.isRiding())
        {
        ...
        if (!Jigsaw.getModuleByName("NoClip").isToggled()
		...
		// TODO Bettersprint
		if (!Jigsaw.getModuleByName("AutoSprint").isToggled() || Jigsaw.disableSprint) {
			if (!this.isSprinting() && this.movementInput.moveForward >= 0.8F && flag4 && !this.isHandActive()
		...
		if (!Jigsaw.getModuleByName("AutoSprint").isToggled() || Jigsaw.disableSprint) {
			if (this.isSprinting()
					&& (this.movementInput.moveForward < 0.8F || this.isCollidedHorizontally || !flag4)) {
        ...
        if (this.isRidingHorse())
        {
			...
			// TODO Jigsaw PerfectHorseJump
			if (Jigsaw.getModuleByName("PerfectHorseJump").isToggled()) {
				this.horseJumpPower = 1;
			}
        }
        else
        {
