...
import ga.ziemniaki.cebulionwarenext.client.modules.BlurBufferMod;
import ga.ziemniaki.cebulionwarenext.client.settings.ClientSettings;
import ga.ziemniaki.cebulionwarenext.framebuffer.ToggleableFramebuffer;
...
public class Minecraft implements IThreadListener, ISnooperInfo
{
    ...
    {
        return Jigsaw.ghostMode ? Jigsaw.fakeClientVersion : this.launchedVersion; //TODO Jigsaw ghostmode
    }
    ...
    private void checkGLError(String message)
    {
        ...
        if (i != 0)
        {
            ...
            if(ClientSettings.enableBlur) {
            	...
            	Jigsaw.getNotificationManager().addNotification(new Notification(Level.ERROR, "OpenGL error, disabled blur effects!"));
            }
        }
    }
    ...
    private void runGameLoop() throws IOException
    {
        ...
        /**/
		//TODO Jigsaw awesome render stuff
		
		//TODO Jigsaw awesome render stuff
		
		if(this.world != null && !Jigsaw.getFramebufferHandler().tfbs.isEmpty()) {
			for(ToggleableFramebuffer buf : Jigsaw.getFramebufferHandler().tfbs) {
				
				if(!buf.isEnabled()) {
					continue;
				}
				
				PreFrameBufferEvent event = new PreFrameBufferEvent();
				buf.getModule().preFrameBuffer(event);
				...
				GlStateManager.pushMatrix();
				buf.frameBuffer.bindFramebuffer(true);

				Jigsaw.getFramebufferHandler().rendering = true;
				
				this.entityRenderer.updateCameraAndRender(this.timer.renderPartialTicks, i);
				
				buf.frameBuffer.unbindFramebuffer();
				GlStateManager.popMatrix();
				
				GlStateManager.pushMatrix();
				
//				buf.frameBuffer.framebufferRenderExt(this.displayWidth / 2 + 100, 100, (int)(this.displayH / 2 + 250), 250, true);
				buf.frameBuffer.framebufferRender(this.displayWidth / 3, this.displayHeight / 3);
				GlStateManager.popMatrix();

				Jigsaw.getFramebufferHandler().rendering = false;
				
				buf.getModule().postFrameBuffer();
			}
		}
		Jigsaw.getFramebufferHandler().overrideThisTick = false;
    ...
    public void clickMouse()
    {
        if (this.leftClickCounter <= 0)
        {
            ...
            else if (!this.player.isRowingBoat())
            {
                ...
                {
                    case ENTITY:
                    	// TODO Jigsaw friends stuff and entityHitEvent
    					EntityHitEvent event = new EntityHitEvent(this.objectMouseOver.entityHit);
    					Jigsaw.onEntityHit(event);
    					if (event.isCancelled()) {
    						break;
    					}
            ...
            if (!this.player.isRowingBoat())
            {
                ...
                for (EnumHand enumhand : EnumHand.values())
                {
                    ...
                    if (this.objectMouseOver != null)
                    {
                        ...
                        {
                            case ENTITY:
            					EntityInteractEvent event = new EntityInteractEvent(this.objectMouseOver.entityHit);
            					Jigsaw.onEntityInteract(event);
            					if (event.isCancelled()) {
            						return;
            					}
    ...
    private void resize(int width, int height)
    {
        ...
        Jigsaw.onResize(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight()); //TODO Jigsaw onresize
    ...
    public void runTick() throws IOException
    {
        ...
        {
        	if (Jigsaw.getModuleByName("FastPlace").isToggled()) { //TODO: Jigsaw FastPlace
    ...
    private void runTickKeyboard() throws IOException
    {
        while (Keyboard.next())
        {
            ...
            if (flag)
            {
                ...
                if (this.currentScreen == null)
                {
					// TODO More Jigsaw Code...
					Jigsaw.onKeyPressed(i);
                    if (i == 1)
                    {
