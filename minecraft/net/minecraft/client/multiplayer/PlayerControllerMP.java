...
public class PlayerControllerMP
{
    ...
    {
    	if(Jigsaw.getModuleByName("ExtendedReach").isToggled()) { return (float) ClientSettings.ExtendedReachrange; } //TODO jigsaw extendedreach
    ...
    public EnumActionResult processRightClickBlock(EntityPlayerSP player, WorldClient worldIn, BlockPos blockPos, EnumFacing enumFacing, Vec3d hitVec, EnumHand enumHand)
    {
        this.syncCurrentPlayItem();
        ...
        else
        {
            if (this.currentGameType != GameType.SPECTATOR)
            {
                ...
                if ((!player.isSneaking() || player.getHeldItemMainhand().func_190926_b() && player.getHeldItemOffhand().func_190926_b()) && iblockstate.getBlock().onBlockActivated(worldIn, blockPos, iblockstate, player, enumHand, enumFacing, f, f1, f2))
                {
                    flag = true;
                }
                ...
                if (!flag && itemstack.getItem() instanceof ItemBlock)
                {
                    ...
                    if (!itemblock.canPlaceBlockOnSide(worldIn, blockPos, enumFacing, player, itemstack))
                    {
                        return EnumActionResult.FAIL;
                    }
                }
            ...
            BlockPlaceEvent event = new BlockPlaceEvent(itemstack, blockPos, enumFacing, hitVec);
			Jigsaw.onBlockPlace(event);
			if (event.isCancelled()) {
				return EnumActionResult.FAIL;
			}
			
            this.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(blockPos, enumFacing, enumHand, f, f1, f2));
            ...
            if (!flag && this.currentGameType != GameType.SPECTATOR)
            {
                ...
                else
                {
                    ...
                    if (this.currentGameType.isCreative())
                    {
                        ...
                        EnumActionResult enumactionresult = itemstack.onItemUse(player, worldIn, blockPos, enumHand, enumFacing, f, f1, f2);
                    ...
                    {
                        return itemstack.onItemUse(player, worldIn, blockPos, enumHand, enumFacing, f, f1, f2);
                    }
                }
            }
            else
    ...
    {
        this.syncCurrentPlayItem();
        Vec3d vec3d = new Vec3d(raytrace.hitVec.x - target.posX, raytrace.hitVec.y - target.posY, raytrace.hitVec.z - target.posZ);
