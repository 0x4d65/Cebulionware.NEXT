...
import ga.ziemniaki.cebulionwarenext.client.tools.LoadTools;
import ga.ziemniaki.cebulionwarenext.font.Fonts;
import ga.ziemniaki.cebulionwarenext.gui.GuiJigsaw;
import ga.ziemniaki.cebulionwarenext.gui.GuiJigsawAltLogin;
import ga.ziemniaki.cebulionwarenext.gui.GuiJigsawAltManager;
import ga.ziemniaki.cebulionwarenext.gui.GuiJigsawChangelog;
import ga.ziemniaki.cebulionwarenext.gui.GuiJigsawCredits;
import ga.ziemniaki.cebulionwarenext.gui.GuiMCLeaksRedeemToken;
...
public class GuiMainMenu extends GuiScreen
{
    ...
    {
    	if(Jigsaw.ghostMode) {
    		drawShitGhost(mouseX, mouseY, partialTicks);
    	}
    	else {
    		float xPlus = (((float) mouseX) - this.width / 2f) / 30f;
			float yPlus = (((float) mouseY) - this.height / 2f) / 30f;
			...
			this.mc.getTextureManager().bindTexture(Jigsaw.menuImage);
			
			GlStateManager.translate(-xPlus / resolution.getScaleFactor(), -yPlus / resolution.getScaleFactor(), 0);
			GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
			GlStateManager.scale(width * 0.0041, height * 0.0041, 0);
			this.drawTexturedModalRect(-10, -7, 0, 0, 512 / 2, 512 / 2);
			...
			drawCenteredString(Fonts.font19, "§rJigsaw currently has §j" + Jigsaw.getModuleCount() + "§r hacks!", width / 2, 5, 0xffffffff);
			
			
			
			{
				Fonts.font18.drawStringWithShadow("§j" + mc.session.getUsername(), 5, 59, 0xffffffff);
				...
				Fonts.font25.drawStringWithShadow("Alt manager", 62, 4, 0xffffffff);
				Fonts.font18.drawStringWithShadow("(Click the head)", 62, 20, 0xffdfdfdf);
			...
			this.drawTexturedModalRect(j + 12, -50, 0, 0, 512 / 2, 512 / 2);

			GL11.glColor4f(1f, 0.95f, 0.95f, 1f);

			this.drawTexturedModalRect(j + 10, -2 + -50, 0, 0, 512 / 2, 512 / 2);
    ...
    public void initGui()
    {
        ...
        this.field_193978_M = this.fontRenderer.getStringWidth("Copyright Mojang AB. Do not distribute!");
        ...
        if(!Jigsaw.ghostMode) {
        	this.buttonList.add(new GuiButton(1, 5, height - 25, 100, 20, "Singleplayer", true));
        	this.buttonList.add(new GuiButton(2, 5 + 100 + 5, height - 25, 100, 20, "Multiplayer", true));
        	this.buttonList.add(new GuiButton(0, 5 + 100 + 5 + 100 + 5, height - 25, 80, 20, "Options", true));
        	
        	this.buttonList.add(new GuiButton(4, width - 70 - 5, height - 25, 70, 20, "Quit Game", true));
    ...
    private void drawShitGhost(int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawString(this.fontRenderer, "Copyright Mojang AB. Do not distribute!", this.field_193979_N, this.height - 10, -1);
