...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void loadShader(ResourceLocation resourceLocationIn, Framebuffer framebuffer)
    {
        if (OpenGlHelper.isFramebufferEnabled())
        {
            try
            {
                this.theShaderGroup = new ShaderGroup(this.mc.getTextureManager(), this.resourceManager, framebuffer, resourceLocationIn);
                this.theShaderGroup.createBindFramebuffers(this.mc.displayWidth, this.mc.displayHeight);
                this.useShader = true;
            }
            catch (IOException ioexception)
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.world != null)
        {
            ...
            Vec3d vec3d3 = null;
            float f = 1.0F;
            List<Entity> list = this.mc.world.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec3d1.x * d0, vec3d1.y * d0, vec3d1.z * d0).expand(1.0D, 1.0D, 1.0D), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>()
            {
    ...
    {
    	if(Jigsaw.getModuleByName("NoHurtcam").isToggled()) {
    		return;
    	}
    ...
    {
        if (!Jigsaw.isFrameBufferRendering() && this.mc.getRenderViewEntity() instanceof EntityPlayer)
        {
    ...
    public void orientCamera(float partialTicks)
    {
        ...
        else if (!this.mc.gameSettings.debugCamEnable)
        {
        	...
        	//TODO Jigsaw orient camera for framebuffer n arrow n stuff
			if(entity instanceof EntityArrow) { 
				if(false) {//IF FIRST PERSON SETTING
					...
					double distance = (double) (this.thirdPersonDistancePrev
							+ (this.thirdPersonDistance - this.thirdPersonDistancePrev) * partialTicks);
					...
					if (this.mc.gameSettings.debugCamEnable) {
						...
						double d4 = (double) (-MathHelper.sin(entityYaw / 180.0F * (float) Math.PI)
								* MathHelper.cos(entityPitch / 180.0F * (float) Math.PI)) * distance;
						double d5 = (double) (MathHelper.cos(entityYaw / 180.0F * (float) Math.PI)
								* MathHelper.cos(entityPitch / 180.0F * (float) Math.PI)) * distance;
						double d6 = (double) (-MathHelper.sin(entityPitch / 180.0F * (float) Math.PI)) * distance;
...
//									.rayTraceBlocks(new Vec3(posX + (double) f3, posY + (double) f4, posZ + (double) f5), new Vec3(
//											posX - d4 + (double) f3 + (double) f5, posY - d6 + (double) f4, posZ - d5 + (double) f5));
//
//							if (movingobjectposition != null) {
//								double d7 = movingobjectposition.hitVec.distanceTo(new Vec3(posX, posY, posZ));
//
//								if (d7 < distance) {
//									distance = d7;
//								}
...
//						}
						...
						GlStateManager.rotate(entity.rotationPitch - entityPitch, 1.0F, 0.0F, 0.0F);
						GlStateManager.rotate(entity.rotationYaw - entityYaw, 0.0F, 1.0F, 0.0F);
						GlStateManager.translate(0.0F, -1.0F, (float) (-distance));
						GlStateManager.rotate(entityYaw - entity.rotationYaw, 0.0F, 1.0F, 0.0F);
						GlStateManager.rotate(entityPitch - entity.rotationPitch, 1.0F, 0.0F, 0.0F);
				...
				if(framebufferHandler.overrideThisTick) {
					float pitch = Jigsaw.getFramebufferHandler().overridePitch;
					float yaw = Jigsaw.getFramebufferHandler().overrideYaw;
					
					GlStateManager.translate(0.0F, 1.0F, -3f);
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        if (!Jigsaw.isFrameBufferRendering() && this.renderHand && !Shaders.isShadowPass)
        {
            if (flag)
            {
