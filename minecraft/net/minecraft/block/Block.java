...
public class Block
{
    ...
    {
		BoundingBoxEvent event = new BoundingBoxEvent(this, pos, entityBox);
		
    	if (Minecraft.getMinecraft().player != null && Minecraft.getMinecraft().player.equals(entityIn)) {
			Jigsaw.onBoundingBox(event);
		}
    	if (event.isCancelled()) {
			return;
		}
    	
        addCollisionBoxToList(pos, event.getBoundingBox(), collidingBoxes, state.getCollisionBoundingBox(worldIn, pos));
    }
