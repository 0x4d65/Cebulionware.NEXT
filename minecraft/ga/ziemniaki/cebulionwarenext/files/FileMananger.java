package ga.ziemniaki.cebulionwarenext.files;
...
import ga.ziemniaki.cebulionwarenext.gui.custom.clickgui.OverlayWindow;
...
public class FileMananger {
	...
	public File jigsawDir = new File(mc.mcDataDir, "cebulionware.NEXT");
	...
	public File fontFile = new File(jigsawDir, "font.ttf");
	public File fontBoldFile = new File(jigsawDir, "fontbold.ttf");
	public File fontGuiFile = new File(jigsawDir, "fontgui.ttf");
	...
	public void saveGUI(ArrayList<ComponentWindow> windows) {
		System.out.println("mc.mcDataDir = " + mc.mcDataDir.getAbsolutePath());
		System.out.println("Saving GUI...");

		JsonObject json = new JsonObject();

		for (ComponentWindow frame : windows) {
			if(frame instanceof OverlayWindow) {
				continue;
			}
			JsonObject jsonFrame = new JsonObject();
			jsonFrame.addProperty("pinned", frame.isPinned());
			jsonFrame.addProperty("enabled", frame.isEnabled());
			jsonFrame.addProperty("extended", frame.isExtended());
	...
	public void loadGUI(ArrayList<ComponentWindow> windows) {
			...
			for (Object s : json.keySet()) {
				...
				for (ComponentWindow frame : windows) {
					try {
						...
						frame.setPinned((Boolean) data.get("pinned"));
						frame.setEnabled((Boolean) data.get("enabled"));
						frame.setExtended((Boolean) data.get("extended"));
