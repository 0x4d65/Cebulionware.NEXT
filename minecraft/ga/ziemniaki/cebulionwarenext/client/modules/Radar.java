...
public class Radar extends Module {
	...
	public Radar() {
		super("Radar", Keyboard.KEY_NONE, Category.RENDER, "An old and ugly radar that I need to redesign some time");
	}
	...
	public void onGui() {
		...
		GL11.glLineWidth(1f);
		float rotation = -((mc.player.prevRotationYawHead
				+ (mc.player.rotationYawHead - mc.player.prevRotationYawHead) * mc.timer.renderPartialTicks));
		for (Entity en : mc.world.loadedEntityList) {
			...
			if (en instanceof EntityPlayer) {
				if (Jigsaw.getFriendsMananger().isFriend((EntityPlayer) en)) {
					...
					if (mc.player.isOnSameTeam((EntityLivingBase) en)) {
