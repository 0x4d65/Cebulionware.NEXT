...
import ga.ziemniaki.cebulionwarenext.client.modules.target.Team;
import ga.ziemniaki.cebulionwarenext.client.settings.ClientSettings;
import ga.ziemniaki.cebulionwarenext.client.tools.RenderTools;
...
//									System.out.println(HackerDetect.getHackerByName(en.getName()).accuracyValue / 360f);
	...
	public void drawESP(float red, float green, float blue, float alpha, Entity e) {
		...
		if(false) {
			if(ClientSettings.espFade) {
				RenderTools.drawSolidEntityESP(xPos, yPos, zPos, e.width / 2, e.height, 1f, 0.5f, 0.5f, Math.min(0.2f, mc.player.getDistanceToEntity(e) / 30f));
				RenderTools.drawOutlinedEntityESP(xPos, yPos, zPos, e.width / 2, e.height, red, green, blue, (mc.player.getDistanceToEntity(e) / 30f));
			...
			if(ClientSettings.espFade) {
				RenderTools.drawSolidEntityESP(xPos, yPos, zPos, e.width / 2, e.height, 0f, 0f, 0f, Math.min(0.3f, mc.player.getDistanceToEntity(e) / 30f));
				RenderTools.drawOutlinedEntityESP(xPos, yPos, zPos, e.width / 2, e.height, red, green, blue, Math.min(0.3f, mc.player.getDistanceToEntity(e) / 30f));
			}
			else {
				RenderTools.drawSolidEntityESP(xPos, yPos, zPos, e.width / 2, e.height, 0f, 0f, 0f, 0.3f);
				RenderTools.drawOutlinedEntityESP(xPos, yPos, zPos, e.width / 2, e.height, red, green, blue, Math.max(0.4f, 1f - (mc.player.getDistanceToEntity(e) / 30f)));
			}
		}
	...
	public static void setESPColor(Entity o) {
		if (!(o instanceof EntityPlayerSP)) {
			if (o instanceof EntityPlayer) {
				EntityPlayer en = (EntityPlayer) o;
				if (Jigsaw.getFriendsMananger().isFriend(en)) {
				...
				if(Utils.isBlacklisted(en)) {
					...
					if(ClientSettings.ESPaccuracyColor) {
						if(HackerDetect.getHackerByName(en.getName()) != null && HackerDetect.getHackerByName(en.getName()).isAccuracyListUsable()) {
							...
							GL11.glColor4f(accuracy, 1f - accuracy / 2f, 1f - accuracy, 1f);
						}
						else {
