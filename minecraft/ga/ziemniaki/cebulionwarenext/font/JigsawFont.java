package ga.ziemniaki.cebulionwarenext.font;
...
public class JigsawFont {
	public int IMAGE_WIDTH = 1024;
	public int IMAGE_HEIGHT = 1024;
	private int texID;
	private final IntObject[] chars = new IntObject['\u00a7'];
	private final Font font;
	private boolean antiAlias;
	private int fontHeight = -1;
	private int charOffset = 8;

	public JigsawFont(Font font, boolean antiAlias, int charOffset) {
		this.font = font;
	...
	public JigsawFont(Font font, boolean antiAlias) {
		this.font = font;
		this.antiAlias = antiAlias;
		this.charOffset = 8;
		setupTexture(antiAlias);
	...
	public void drawChar(char c, float x, float y) throws ArrayIndexOutOfBoundsException { //TODO fix font shit
		try {
			drawQuad(x, y, this.chars[c].width, this.chars[c].height, this.chars[c].storedX, this.chars[c].storedY,
					this.chars[c].width, this.chars[c].height);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	private void drawQuad(float x, float y, float width, float height, float srcX, float srcY, float srcWidth,
			float srcHeight) {
		...
		GlStateManager.glTexCoord2f(renderSRCX + renderSRCWidth, renderSRCY);
		GL11.glVertex2d(x + width, y);
		GlStateManager.glTexCoord2f(renderSRCX, renderSRCY);
		GL11.glVertex2d(x, y);
		GlStateManager.glTexCoord2f(renderSRCX, renderSRCY + renderSRCHeight);
		GL11.glVertex2d(x, y + height);
		GlStateManager.glTexCoord2f(renderSRCX, renderSRCY + renderSRCHeight);
		GL11.glVertex2d(x, y + height);
		GlStateManager.glTexCoord2f(renderSRCX + renderSRCWidth, renderSRCY + renderSRCHeight);
