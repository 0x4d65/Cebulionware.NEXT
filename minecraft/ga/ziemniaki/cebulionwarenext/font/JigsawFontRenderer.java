...
public class JigsawFontRenderer extends FontRenderer {
	...
	private String colorcodeIdentifiers = "0123456789abcdefklmnorj";
	...
	public int drawString(String text, float x, float y, int color, boolean shadow) {
		...
		for (int i = 0; i < lines.length; i++) {
			result = drawLine(lines[i], x, y + i * getHeight(), new Color(red, green, blue, alpha), shadow);
		}
		return result;
	}
	...
	public int drawString(String text, int x, int y, Color color) {
		...
		String[] lines = text.split("\n");
		for (int i = 0; i < lines.length; i++) {
			result = drawLine(lines[i], x, y + i * getHeight(), color, false);
		}
		return result;
	}
	...
	public int drawString(String text, double x, double y, Color color) {
		...
		String[] lines = text.split("\n");
		for (int i = 0; i < lines.length; i++) {
			result = drawLine(lines[i], (float)x, (float)y + i * getHeight(), color, false);
		}
		return result;
	}
	
	private int drawLine(String text, float x, float y, Color c, boolean shadow) {
		if (text == null) {
			return 0;
		}
					...
					if (index == 0) {
						...
						if (colorIndex != -1) {
							if (colorIndex < 16) {
								...
								currentColor = ClientSettings.getForeGroundGuiColor().brighter();
							} else if (colorIndex > 22) {
	...
	private String setupColorcodeIdentifier() {
		String minecraftColorCodes = "0123456789abcdefklmnorj";
