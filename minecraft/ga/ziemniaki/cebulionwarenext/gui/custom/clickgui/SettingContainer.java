...
import ga.ziemniaki.cebulionwarenext.gui.custom.clickgui.layout.HorizontalLayout;
	...
	public void draw() {
		...
		if(!isFullyClosed()) {
			...
			glBegin(GL_QUADS);
			{
				...
				glVertex2d(getBorderX(), getHeight());
				glVertex2d(getBorderX(), 0);
			}
			glEnd();
			glBegin(GL_QUADS);
			{
				glVertex2d(getWidth() - getBorderX(), 0);
				glVertex2d(getWidth() - getBorderX(), getHeight());
			...
			{
				glVertex2d(getBorderX(), 0);
				glVertex2d(getBorderX(), getBorderY());
				glVertex2d(getWidth() - getBorderX(), getBorderY());
				glVertex2d(getWidth() - getBorderX(), 0);
			}
			glEnd();
			glBegin(GL_QUADS);
			{
				glVertex2d(getBorderX(), getHeight() - getBorderY());
				glVertex2d(getBorderX(), getHeight());
				glVertex2d(getWidth() - getBorderX(), getHeight());
				glVertex2d(getWidth() - getBorderX(), getHeight() - getBorderY());
			}
			glEnd();
		}
