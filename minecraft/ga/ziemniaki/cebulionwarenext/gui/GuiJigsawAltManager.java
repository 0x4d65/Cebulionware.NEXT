...
import ga.ziemniaki.cebulionwarenext.client.alts.Alt;
import ga.ziemniaki.cebulionwarenext.client.alts.Login;
...
import ga.ziemniaki.cebulionwarenext.gui.animations.Animation;
...
public class GuiJigsawAltManager extends GuiScreen {
	...
	Animation anim = new Animation() {
		...
		public void render() {
			...
			bufferBuilder.pos((double) 0, (double) height + 1, 0.0D).endVertex();
			bufferBuilder.pos((double) width, (double) height + 1, 0.0D).endVertex();
			bufferBuilder.pos((double) width, (double) 0, 0.0D).endVertex();
			bufferBuilder.pos((double) 0, (double) 0, 0.0D).endVertex();
			tessellator.draw();
	...
	public void initGui() {
		...
		this.buttonList.add(new GuiButton(0, width / 2 - 90, height - 44 - 5 + 26, 88, 20, "Login Selected", true));
		this.buttonList.add(new GuiButton(1, width / 2 + 0, height - 44 - 5 + 26, 88, 20, "Remove Selected", true));
		this.buttonList.add(new GuiButton(2, 3, 3, 88, 20, "Direct Login", true));
		this.buttonList.add(new GuiButton(3, width / 2 + 90, height - 44 - 5 + 26, 48, 20, "Back", true));
		this.buttonList.add(new GuiButton(4, width / 2 - 140, height - 44 - 5 + 26, 48, 20, "Add Alt", true));
		this.buttonList.add(new GuiButton(5, width - 135, 5, 133, 20, "Import from Accountlist", true));
		this.buttonList.add(new GuiButton(6, width - 135, 28, 133, 20, "Export to Accountlist", true));
		
		this.buttonList.add(new GuiButton(10, this.width / 2 + 1 + 100, this.height / 2, 48, 15, "Add", true));
		
		this.buttonList.add(new GuiButton(11, this.width / 2 - 50, this.height - 80, 100, 20, "Okay", true));
		
		this.buttonList.get(7).visible = false;
		this.buttonList.get(8).visible = false;

		searchField = new GuiTextField(7, Fonts.font18, width / 2 - 50, 50, 98, 15);
		searchField.setMaxStringLength(666);
		searchField.setFocused(true);

		emailField = new GuiTextField(8, Fonts.font18, this.width / 2 - 99 - 50, this.height / 2, 145, 15);
		emailField.setMaxStringLength(666);

		passField = new GuiTextField(9, Fonts.font18, this.width / 2 + 1, this.height / 2, 95, 15);
		...
		this.buttonList.add(new GuiButton(69, 3, 3 + 23, 88, 20, "§kA§r Buy Alts §kA§r", true, false));
	}
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if(button.id == 0) {
			if(selected < 0) {
				if(!Jigsaw.getAltManager().getAlts().isEmpty()) {
					selected = 0;
				}
				return;
			}
					...
					try {
						...
						while ((line = br.readLine()) != null) {
							...
							if(line.split(":").length > 1) {
								pass = line.split(":")[1].trim().replaceAll("\n", "").replaceAll("\r", "");
							}
		...
		if(button.id == 69) {
			try {
				openWebLink(new URI("https://selly.gg/u/ProtonAlts"));
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(Fonts.font18, Jigsaw.headerNoBrackets, width / 2 / 2, (10) / 4, 0xffffffff);
		drawCenteredString(Fonts.font18, "§7Alt Manager", this.width / 2 / 2, (49 - 25) / 2, 0xffffffff);
		...
		for (Alt alt : Jigsaw.getAltManager().getAlts()) {
			...
			drawHorizontalLine(left, right - 1, top, 0xffffff);
			drawHorizontalLine(left, right - 1, bottom - 1, 0xffffff);
			drawVerticalLine(left, top, bottom, 0xffffff);
			drawVerticalLine(right - 1, top, bottom, 0xffffff);
			...
			if(Jigsaw.loggedInName != null && Jigsaw.loggedInName.equals(alt.name)) {
				drawString(Fonts.font18, "§2Logged in", right - Fonts.font18.getStringWidth("§2Logged in") - 2, bottom - 18, 0xffffffff);
			}
			
			drawString(Fonts.font18, alts + 1 + ".", left - Fonts.font18.getStringWidth(alts + 1 + ".") - 2, top + 6, 0xffffffff);
			drawCenteredString(Fonts.font18, alt.cracked ? "§r" + alt.name : "§r" + alt.name + "§r:" + "§8" + obfPass, width / 2, top + 6, 0xffffffff);
		...
		drawRect(0, height - 25, width, height, GuiUtils.toARGB(ClientSettings.getForeGroundGuiColor()));
		
		super.drawScreen(mouseX, mouseY, partialTicks);
		anim.render();
		if(failed) {
			GlStateManager.scale(4, 4, 1);
			drawCenteredString(Fonts.font18, "§cFailed to login!", this.width / 2 / 4, (60) / 4, 0xffffffff);
			GlStateManager.scale(0.25, 0.25, 1);
			drawCenteredString(Fonts.font18, "§7" + failMessage, this.width / 2, 100, 0xffffffff);
			
			((GuiButton) this.buttonList.get(8)).drawButton(this.mc, mouseX, mouseY, mc.timer.renderPartialTicks);
		}
		if (displayingLogin) {
			GlStateManager.scale(4, 4, 1);
			drawCenteredString(Fonts.font18, "§cAdd §8alt", this.width / 2 / 4, (60) / 4, 0xffffffff);
			GlStateManager.scale(0.25, 0.25, 1);
			drawString(Fonts.font18, "Email/Username", this.width / 2 - 99 - 50, this.height / 2 - 11, 0xffffffff);
			drawString(Fonts.font18, "Password", this.width / 2 + 1, this.height / 2 - 11, 0xffffffff);

			emailField.drawTextBox();
			passField.drawTextBox();

			((GuiButton) this.buttonList.get(7)).drawButton(this.mc, mouseX, mouseY, mc.timer.renderPartialTicks);
			((GuiButton) this.buttonList.get(3)).drawButton(this.mc, mouseX, mouseY, mc.timer.renderPartialTicks);
		}
