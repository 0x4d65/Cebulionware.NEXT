...
public class GuiJigsawBypassList extends GuiScreen {
	...
	public void initGui() {
		...
		for(int i = 0; i < Jigsaw.getPresetManager().getBypasses().size(); i++) {
			this.buttonList.add(new GuiButton(i + 1, this.width / 2 - 100, height / 2 + (i * 22), 200, 20, Jigsaw.getPresetManager().getBypasses().get(i).getName()));
		}
	}
...
//			Jigsaw.getBypassManager().enableBypass(button.displayString);
		}
	}
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(fontRenderer, "§7Bypasses List", this.width / 2 / 2, (this.height / 2 - 25 - offset) / 2, 0xffffffff);
		GlStateManager.scale(0.5, 0.5, 1);
