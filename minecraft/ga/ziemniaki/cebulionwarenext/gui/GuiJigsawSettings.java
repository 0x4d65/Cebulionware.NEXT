...
public class GuiJigsawSettings extends GuiScreen {
	...
	public void initGui() {
		this.buttonList.add(new GuiButton(7, this.width / 2 - 80, this.height / 2 - offset, 160, 20, "Remove all Keybinds"));
		
		this.buttonList.add(new GuiButton(0, this.width / 2 - 80, this.height / 2 + 22 - offset, 160, 20, "Reset all Keybinds"));
		
		this.buttonList.add(new GuiButton(1, this.width / 2 - 80, this.height / 2 + 44 - offset, 160, 20, "Reset all Friends"));

		this.buttonList.add(new GuiButton(3, this.width / 2 - 100, height - 50, 200, 20, "Done"));

		this.buttonList.add(new GuiButton(4, this.width / 2 - 80, this.height / 2 + 66 - offset, 160, 20, "Change ClickGUI Keybind"));
		
		this.buttonList.add(new GuiButton(5, this.width / 2 - 80, this.height / 2 + 88 - offset, 160, 20, ClientSettings.bigWaterMark ? "Watermark: Big" : "Watermark: Small"));
		
		this.buttonList.add(new GuiButton(6, this.width / 2 - 80, this.height / 2 + 110 - offset, 160, 20, ClientSettings.tabGui ? "TabGui: Enabled" : "TabGui: Disabled"));
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		if (button.id == 2) {
			for (EntityPlayer en : mc.world.playerEntities) {
				if (mc.player.getDistanceToEntity(en) <= 24) {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(fontRenderer, "§7Settings", this.width / 2 / 2, (this.height / 2 - 25 - offset) / 2, 0xffffffff);
		GlStateManager.scale(0.5, 0.5, 1);
