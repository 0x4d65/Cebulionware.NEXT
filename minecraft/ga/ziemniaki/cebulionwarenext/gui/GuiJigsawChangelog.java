	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (!Jigsaw.changelogFailed) {
			if (Jigsaw.changeLineElmts != null) {
				...
				drawString(Fonts.font18, "Changelog: ", 2, (int) Math.round(30 - scroll / 1.8), 0xffffff);
				GlStateManager.popMatrix();
				int sub = 80;
				for (Element elmt : Jigsaw.changeLineElmts) {
					if (elmt.ownText().trim().equals(Jigsaw.getClientVersion())) {
						drawString(Fonts.font18, "§c§l" + elmt.ownText() + "§6 - Your version", 5, sub - scroll,
								0xffffff);
					} else if (elmt.ownText().trim().equals(Jigsaw.serverVersion)) {
						drawString(Fonts.font18, "§e§l" + elmt.ownText() + "§6 - Latest version", 5, sub - scroll,
								0xffffff);
					} else {
						drawString(Fonts.font18, "§l" + elmt.ownText(), 5, sub - scroll, 0xffffff);
					}
				...
				drawString(Fonts.font18, "§7Loading changelog...", 4, height / 2 - 30 - scroll, 0xffffff);
			}
		} else {
			drawString(Fonts.font18, "§cCould not get changelog!", 4, height / 2 - 30 - scroll, 0xffffff);
		}
	...
	public void handleMouseInput() throws IOException {
		...
		if (i != 0) {
			...
			for (int ii = 0; ii < this.buttonList.size(); ++ii) {
				((GuiButton) this.buttonList.get(ii)).y += i;
			}
		}
	}
