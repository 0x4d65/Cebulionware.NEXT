package ga.ziemniaki.cebulionwarenext.cracker;
...
public class CrackManager {
	...
	/*
	 * String path = "/me/robbanrobbin/assets/cracker/passlist.txt"; URL url =
	 * getClass().getResource(path); try { File file = new File(url.toURI());
	 * passwords = FileUtils.readLines(file, Charset.forName("UTF-8")); } catch
	 * (URISyntaxException e) { e.printStackTrace(); } catch (IOException e) {
	 * e.printStackTrace(); }
	 * 
	 * path = "/me/robbanrobbin/assets/cracker/proxies3.txt"; url =
	 * getClass().getResource(path); try { File file = new File(url.toURI());
	 * proxies = FileUtils.readLines(file, Charset.forName("UTF-8")); } catch
	 * (URISyntaxException e) { e.printStackTrace(); } catch (IOException e) {
	 * e.printStackTrace(); } }
	 */
	...
	public void onGui() {
		if (mc.currentScreen == null) {
			Jigsaw.getUIManager().addToQueue("§6Crack §rattempts: " + tries, ScreenPos.LEFTUP);
			Jigsaw.getUIManager().addToQueue("§6Crack §rbad proxies: " + badProxies, ScreenPos.LEFTUP);
			Jigsaw.getUIManager().addToQueue("§6Crack §rfinished: " + stop, ScreenPos.LEFTUP);
			Jigsaw.getUIManager().addToQueue("§6Crack §rsuccess: " + hacked, ScreenPos.LEFTUP);
		}
	}
}
