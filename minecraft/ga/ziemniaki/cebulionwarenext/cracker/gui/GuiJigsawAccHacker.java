package ga.ziemniaki.cebulionwarenext.cracker.gui;
...
public class GuiJigsawAccHacker extends GuiScreen {
	...
	public void initGui() {
		...
		emailField = new GuiTextField(2, this.fontRenderer, this.width / 2 - 100, this.height / 2, 200, 20);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawString(fontRenderer, "§7Status: " + status, 10, 10, 0xffffffff);
		if (Jigsaw.getCrackManager() != null) {
			drawString(fontRenderer, "§7Tries: " + Jigsaw.getCrackManager().tries, 10, 20, 0xffffffff);
			drawString(fontRenderer, "§7Bad Proxies: " + Jigsaw.getCrackManager().badProxies, 10, 30, 0xffffffff);
			drawString(fontRenderer, "§7Credentials: "
					+ (Jigsaw.getCrackManager().emailPass == null ? "unknown" : Jigsaw.getCrackManager().emailPass), 10,
					40, 0xffffffff);
		}
		GlStateManager.scale(4, 4, 1);
		drawCenteredString(fontRenderer, Jigsaw.headerNoBrackets, this.width / 2 / 4, (this.height / 2 - 67 - 25) / 4, 0xffffffff);
		GlStateManager.scale(0.5, 0.5, 1);
		drawCenteredString(fontRenderer, "§7Account Cracker", this.width / 2 / 2, (this.height / 2 - 25 - 25) / 2, 0xffffffff);
		GlStateManager.scale(0.5, 0.5, 1);
		drawHorizontalLine((this.width / 2 - 60) / 1, (this.width / 2 - 80 + 138) / 1, (this.height / 2 - 5 - 25) / 1, 0xffaaaaaa);
		
		drawCenteredString(fontRenderer, "§cThis tries the 10 most common passwords", this.width / 2 / 1, (this.height / 2 - 22) / 1, 0xffffffff);
		drawString(fontRenderer, "§7E-mail:", this.width / 2 - 100, (this.height / 2 - 11) / 1, 0xffffffff);
